
// import img2 from '../images/events/Meme_Live.webp';
// import img3 from '../images/events/Poster_Live.webp';
// import img4 from '../images/events/Photography_Live.webp';
// import img5 from '../images/events/TechEvent_Live.webp';
// import img6 from '../images/events/Content_Live.webp';
// import img7 from '../images/events/Video_Live.webp';
// import img8 from '../images/events/hacksamarth23.webp';
// import img9 from '../images/events/chronicles-23.webp';
// import img10 from '../images/events/PHOTOGRAPHY-24.webp';
// import img11 from '../images/events/TOS-24.webp';
// import img12 from '../images/events/COMING SOON.webp';
import img13 from '/images2/events/instant_ink.webp';
import img14 from '/images2/events/MATRiX OF MOCK 1080 (1).webp';
import img15 from '/images2/events/1080 Apti new.webp';
import img16 from '/images2/events/shot a vid (2).webp';
import img17 from '/images2/events/INNOVATHON final (5).webp';
import img18 from '/images2/events/package.webp';
// import img19 from '../images/events/demystifying_seminar.webp';
// import img20 from '../images/events/personal_interview.webp';
// import img11 from '../images/events/War of conviction.webp';

const dataItem5 = [
  // {
  //   id: 0,
  //   img: img11,
  //   title: 'TITANS OF SITUATION',     //TOS-24
  //   ownedBy: 'Samarth',
  //   link: 'https://forms.gle/2uRZubSXKhGw8YpM8', 
  // },  
  // {
  //   id: 1,
  //   img: img10,
  //   title: 'PHOTOGRAPHY',
  //   ownedBy: 'PRATIBIMB-Photography Wing',
  //   link: 'https://docs.google.com/forms/d/e/1FAIpQLSeACC9K2H8mdjjOJB4b1-E42kmpf-zi-eUONGb5s3o9eqed3g/viewform?usp=sf_link', 
  // },  
  // {
  //   id: 2,
  //   img: img9,
  //   title: 'Showcase Your Talent',
  //   ownedBy: 'Samarth',
  //   link: 'https://forms.gle/j76b8RmxjGmHrNQQ8', 
  // },  
  // {
  //   id: 3,
  //   img: img8,
  //   title: 'Hacktoberfest,23',
  //   ownedBy: 'PRAVIDHI-Tech Wing',
  //   link: 'https://linktr.ee/SamarthXHacktoberfest23', 
  // },  
  // {
  //       id: 4,
  //       img: img2,
  //       title: 'Meme',
  //       ownedBy: 'Samarth',
  //       link: 'https://forms.gle/Pf8Mtivm1CW91hyS9', 
  //     },
  //     {
  //       id: 5,
  //       img: img3,
  //       title: 'Poster',
  //       ownedBy: 'KALKRITI-Graphic Wing',
  //       link: 'https://forms.gle/Pf8Mtivm1CW91hyS9', 
  //     },
  //     {
  //       id: 6,
  //       img: img4,
  //       title: 'Photography',
  //       ownedBy: 'PRATIBIMB-Photography Wing',
  //       link: 'https://forms.gle/Pf8Mtivm1CW91hyS9', 
  //     },
  //     {
  //       id: 7,
  //       img: img5,
  //       title: 'Tech Event',
  //       ownedBy: 'PRAVIDHI-Tech Wing',
  //       link: 'https://forms.gle/Pf8Mtivm1CW91hyS9', 
  //     },
  //     {
  //       id: 8,
  //       img: img6,
  //       title: 'Content',
  //       ownedBy: 'PRAKARAN-Editorial Wing',
  //       link: 'https://forms.gle/Pf8Mtivm1CW91hyS9', 
  //     },
  // {
  //   id: 9,
  //   img: img7,
  //   title: 'Video',
  //   ownedBy: 'KALKRITI-Graphic Wing',
  //   link: 'https://forms.gle/Pf8Mtivm1CW91hyS9', 
  // },
  //   {
  //     id: 7,
  //     img: img8,
  //     title: 'Model United State',

  //     ownedBy: 'ANVESHAN-Research Wing',
  //   },
  //   {
  //     id: 8,
  //     img: img9,
  //     title: 'BRAIN BLITZ',

  //     ownedBy: 'GYAN DARPAN-Eductation Wing',
  //   },
  //   {
  //     id: 9,
  //     img: img10,
  //     title: 'DREAM DOTS',

  //     ownedBy: 'SANGAM-Cultural Wing',
  //   },

  //   {
  //     id: 10,
  //     img: img11,
  //     title: 'War Of Convictions',

  //     ownedBy: 'GYAN DARPAN-Eductation Wing',

  //   },
  // {
  //   id: 17,
  //   img: img19,
  //   title: 'DEMYSTIFYING SEMINAR',
  //   ownedBy: 'Samarth',
  //   link: 'https://forms.gle/WVBvBnaLyx2Wj92LA',
  // },
  // {
  //   id: 18,
  //   img: img20,
  //   title: 'Unlocking Success : Mastering the Personal Interview',
  //   ownedBy: 'Samarth',
  //   link: 'https://forms.gle/7QSou2Nk8duYJg9z7',
  // },

  // **Safalya '24**
  {
    id: 11,
    img: img13,
    title: 'INSTANT INK',
    ownedBy: 'Samarth',
    link: 'https://docs.google.com/forms/d/e/1FAIpQLSdq0-7nMvIWw8Ql5UN_TK4qA9ldWyCeLqDaKUGJ02t_kkF3Jg/viewform',
  },
  {
    id: 13,
    img: img15,
    title: 'APTI ACUMEN',
    ownedBy: 'Samarth',
    link: 'https://docs.google.com/forms/d/e/1FAIpQLSdq0-7nMvIWw8Ql5UN_TK4qA9ldWyCeLqDaKUGJ02t_kkF3Jg/viewform',
  },
  {
    id: 12,
    img: img14,
    title: 'MATRIX OF MOCK',
    ownedBy: 'Samarth',
    link: 'https://docs.google.com/forms/d/e/1FAIpQLSdq0-7nMvIWw8Ql5UN_TK4qA9ldWyCeLqDaKUGJ02t_kkF3Jg/viewform',
  },
  {
    id: 14,
    img: img16,
    title: 'THOUGHTS IN SHORTS',
    ownedBy: 'Samarth',
    link: 'https://docs.google.com/forms/d/e/1FAIpQLSdq0-7nMvIWw8Ql5UN_TK4qA9ldWyCeLqDaKUGJ02t_kkF3Jg/viewform',
  },
  {
    id: 15,
    img: img17,
    title: 'INNOVATE-A-THON',
    ownedBy: 'Samarth',
    link: 'https://docs.google.com/forms/d/e/1FAIpQLSdq0-7nMvIWw8Ql5UN_TK4qA9ldWyCeLqDaKUGJ02t_kkF3Jg/viewform',
  },
  {
    id: 16,
    img: img18,
    title: 'PACKAGE OFFER-ALL EVENTS',
    ownedBy: 'Samarth',
    link: 'https://docs.google.com/forms/d/e/1FAIpQLSdq0-7nMvIWw8Ql5UN_TK4qA9ldWyCeLqDaKUGJ02t_kkF3Jg/viewform',
  },

];

export default dataItem5;
